...
class Game:
    ...
    def move_promotion(self, window, current_move, turn):
        is_white = True if self.is_player_white and turn == 'player' else False
        if current_move == 'a7a8':                                                           # Player turn
            self.board_array[1][0] = constants.BLANK                                         # Clear Pawn
            self.board_array[0][0] = constants.QUEEN_W if is_white else constants.QUEEN_B    # Set Queen
        elif current_move == 'b7b8':                               
            self.board_array[1][1] = constants.BLANK                                         # Clear Pawn
            self.board_array[0][1] = constants.QUEEN_W if is_white else constants.QUEEN_B    # Set Queen
        elif current_move == 'c7c8':                               
            self.board_array[1][2] = constants.BLANK                                         # Clear Pawn
            self.board_array[0][2] = constants.QUEEN_W if is_white else constants.QUEEN_B    # Set Queen
        elif current_move == 'd7d8':                               
            self.board_array[1][3] = constants.BLANK                                         # Clear Pawn
            self.board_array[0][4] = constants.QUEEN_W if is_white else constants.QUEEN_B    # Set Queen
        elif current_move == 'e7e8':                               
            self.board_array[1][4] = constants.BLANK                                         # Clear Pawn
            self.board_array[0][4] = constants.QUEEN_W if is_white else constants.QUEEN_B    # Set Queen
        elif current_move == 'f7f8':                               
            self.board_array[1][5] = constants.BLANK                                         # Clear Pawn
            self.board_array[0][5] = constants.QUEEN_W if is_white else constants.QUEEN_B    # Set Queen
        elif current_move == 'g7g8':                               
            self.board_array[1][6] = constants.BLANK                                         # Clear Pawn
            self.board_array[0][6] = constants.QUEEN_W if is_white else constants.QUEEN_B    # Set Queen
        elif current_move == 'h7h8':                               
            self.board_array[1][7] = constants.BLANK                                         # Clear Pawn
            self.board_array[0][7] = constants.QUEEN_W if is_white else constants.QUEEN_B    # Set Queen 
        gui.update_board(window, self.dark_sq, self.light_sq, self.board_array)              # Draw Changes
    
    def move_handler(self, window, board, player, move, original_sq, new_sq, current_piece, origin_row):
        """
            ...
            if type(button) is tuple:                                                 # If chess_board button is clicked
                if not has_user_moved:
                    ...
                    if player_move in board.legal_moves:                      # Check if turn move is legal
                        self.move_handler(window, board, 'player', player_move, original_sq, new_sq, current_piece, origin_row)
    ...
    def engine_move(self, window, board, move_count, timer_player, timer_engine):
        ...
        self.move_handler(window, board, 'engine', engine_move, original_sq, new_sq, current_piece, origin_row)

        elapse_str = timer.get_timer(timer_engine.elapsed)
    ...
    def game_controller(self):  # Begin chess game
        ...
        if timer_player.run_out == True and timer_player.base <= 0 and self.is_timer_on == True:
            ...
            is_exit_game = True
            result = 'Engine Wins!'
            result_str = 'Time out! Result: ' + result
            ...
            if self.chess_board.is_game_over(claim_draw=True) :
                ...
                result_str = 'Match End! Result: ' + result
                sg.Popup(result_str, font='helvetica', title=constants.WINDOW_TITLE)
